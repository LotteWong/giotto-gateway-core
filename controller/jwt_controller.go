...
func (c *JwtController) GenerateJwt(ctx *gin.Context) {
	...
	if len(cipherInfo) != 2 {
		middleware.ResponseError(ctx, 6001, errors.New("can not get app id and secret from authorization header"))
		return
	}

	plainInfo, err := base64.StdEncoding.DecodeString(cipherInfo[1])
	if err != nil {
		middleware.ResponseError(ctx, 6002, errors.New(fmt.Sprintf("base64 decode app id and secret error: %v", err)))
		return
	}

	pair := strings.Split(string(plainInfo), ":")
	if len(pair) != 2 {
		middleware.ResponseError(ctx, 6003, errors.New("can not get app id and secret from authorization header"))
		return
	}
	...
	if err != nil {
		middleware.ResponseError(ctx, 6004, err)
		return
	}
	res, err := service.GetJwtService().GenerateJwt(ctx, tx, req, appId, secret)
	if err != nil {
		middleware.ResponseError(ctx, 6005, err)
		return
	}
